...
-- สอบถาม ผู้ให้บริการ
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' ,1,NULL)) 'Question.hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'การชดเชยค่าบริการ' ,1,NULL)) 'Questionchocheykaborikarn.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'ข้อมูลหน่วยงาน' ,1,NULL)) 'Questionkoermunennuanngn.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'ข้อมูลและโครงสร้างองค์กร' ,1,NULL)) 'Questioninformationongkorn.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'ข้อมูลหน่วยบริการ/สถานบริการ' ,1,NULL)) 'Questioninformationservice.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'ขึ้นทะเบียน/ออกจากการเป็นหน่วยบริการ' ,1,NULL)) 'Questionkeungtabuean.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'ตรวจสอบข้อมูลสิทธิ' ,1,NULL)) 'Questioncheckinformation.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'ตามเรื่อง' ,1,NULL)) 'Questionfollowsubject.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'ลงทะเบียน' ,1,NULL)) 'QusetionRegister.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'ระบบ/โปรแกรม' ,1,NULL)) 'Qusetionsystem/program.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'สิทธิประโยชน์และวิธีการใช้สิทธิ' ,1,NULL)) 'Qusetionsybenefitsandmethodright.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'สิทธิไม่ตรงตามจริง' ,1,NULL)) 'Qusetionsrightnouttrue.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'โอนสาย' ,1,NULL)) 'Qusetiontransfer.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'สอบถาม' and t.infor = 'อื่นๆ' ,1,NULL)) 'Qusetionetc.Hospital',
...
-- ร้องเรียนมาตรา 57,59 ผู้ให้บริการ
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ร้องเรียน'  ,1,NULL)) 'Appeal.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ร้องเรียน' and t.sub_topic = 'ไม่ได้รับความสะดวกตามสมควร' ,1,NULL)) 'Appealmaidairubkwabsadueak.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ร้องเรียน' and t.sub_topic = 'ไม่ได้รับบริการตามสิทธิที่กำหนด' ,1,NULL)) 'Appealmaidairubborikarn.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ร้องเรียน' and t.sub_topic = 'มาตรฐานการให้บริการสาธารณสุข' ,1,NULL)) 'Appealstandard.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ร้องเรียน' and t.sub_topic = 'ถูกเรียกเก็บเงิน' ,1,NULL)) 'AppealTukreakkebmoney.Hospital',
...
-- ร้องทุกข์ ผู้ให้บริการ
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ร้องทุกข์'  ,1,NULL)) 'Complain.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')   and  t.topic = 'ร้องทุกข์'  and t.sub_topic = 'สถานบริการอื่นปฏิเสธให้สิทธิเจ็บป่วยฉุกเฉิน' ,1,NULL)) 'Complaindenyright.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')   and  t.topic = 'ร้องทุกข์'  and t.sub_topic = 'การลงทะเบียน' ,1,NULL)) 'ComplainRegister.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ร้องทุกข์'  and t.sub_topic = 'สิทธิไม่ตรงตามจริง' ,1,NULL)) 'Complainrightnottrue.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.') and  t.topic = 'ร้องทุกข์'  and t.sub_topic = 'ขอความช่วยเหลือ' ,1,NULL)) 'Complainhelp.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')   and  t.topic = 'ร้องทุกข์'  and t.sub_topic = 'อื่นๆ' ,1,NULL)) 'Complainetc.Hospital',
...
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ร้องเรียนบุคลากร สปสช.' ,1,NULL)) 'Appealperson.Hospital',

-- บัตรสนเท่ห์
count(if(c.typecont in ('ประชาชน','ภาคีเครือข่าย')  and  t.topic = 'บัตรสนเท่ห์' ,1,NULL)) 'budsontey.Service',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'บัตรสนเท่ห์' ,1,NULL)) 'budsontey.Hospital',

-- การบริการทั่วไป
count(if(c.typecont in ('ประชาชน','ภาคีเครือข่าย')  and  t.topic = 'การบริการ' ,1,NULL)) 'Service.Service',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'การบริการ' ,1,NULL)) 'Service.Hospital',

-- เสนอแนะ
count(if(c.typecont in ('ประชาชน','ภาคีเครือข่าย')  and  t.topic = 'เสนอแนะ' ,1,NULL)) 'Suggestion.Service',
count(if(c.typecont in ('โรงพยาบาล','อปท.')   and  t.topic = 'เสนอแนะ' ,1,NULL)) 'Suggestion.Hospital',

-- ปรึกษา
count(if(c.typecont in ('ประชาชน','ภาคีเครือข่าย')  and  t.topic = 'ปรึกษา' ,1,NULL)) 'Consult.Service',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ปรึกษา' ,1,NULL)) 'Consult.Hospital',

-- ชมเชย
count(if(c.typecont in ('ประชาชน','ภาคีเครือข่าย')  and  t.topic = 'ชมเชย' ,1,NULL)) 'Commend.Service',
count(if(c.typecont in ('โรงพยาบาล','อปท.')   and  t.topic = 'ชมเชย' ,1,NULL)) 'Commend.Hospital',
...
-- ประสานหาเตียง ผู้ให้บริการ
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ประสานหาเตียง' ,1,NULL)) 'Parsarnhabed.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ประสานหาเตียง' and t.sub_topic = 'หน่วยบริการ' and t.subco_topic = 'เตียงเต็ม' ,1,NULL)) 'Parsarnhabedbedfull.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')  and  t.topic = 'ประสานหาเตียง' and t.sub_topic = 'หน่วยบริการ' and t.subco_topic = 'เกินศักยภาพ' ,1,NULL)) 'Parsarnhabedoverperformance.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')   and  t.topic = 'ประสานหาเตียง' and t.sub_topic = 'หน่วยบริการ' and t.subco_topic = 'ประสานกลับต้นสังกัด' ,1,NULL)) 'Parsarnhabedtonsankud.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')   and  t.topic = 'ประสานหาเตียง' and t.sub_topic = 'รพ.ไม่เข้าร่วมโครงการ',1,NULL)) 'Parsarnhabedhospitalnotentrance.Hospital',
count(if(c.typecont in ('โรงพยาบาล','อปท.')   and  t.topic = 'ประสานหาเตียง' and t.sub_topic = '',1,NULL)) 'Parsarnhabedetc.Hospital'
...
where
t.right_u_c_e_p = 'สิทธิหลักประกันสุขภาพแห่งชาติ (ผู้ประกันตนคนพิการ)' and
...
order by monthyear,month(t.created_at)