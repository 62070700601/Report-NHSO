CREATE DEFINER=`dev`@`%` PROCEDURE `agent_performance_login_test`(IN `start_date_param` DATE, IN `end_date_param` DATE,IN `business_units_param` TEXT, IN `team_param` TEXT, IN `agent_param` TEXT)
BEGIN
WITH
...
order by date(`task-records`.started_at), business_units, teams, agent
    ...
    allvoice(user_uuid,business_units,team_uuid,teams,agent,daily,handled_voice, handledtime_voice, handledtime_voice_min, handledtime_voice_max)
    AS (
		SELECT 
    ...
    allchat(daily,handled_chat, handledtime_chat, handledtime_chat_min, handledtime_chat_max, user_uuid,team_uuid)
    AS (
	SELECT 
    date(`agent-task-records`.started_at) as daily,
    ...
    allemail(daily,handled_email, handledtime_email, handledtime_email_min, handledtime_email_max, user_uuid,team_uuid)
    AS (
	SELECT 
    date(`agent-task-records`.started_at) as daily,
    ...
    allsocial(daily,handled_social, handledtime_social, handledtime_social_min, handledtime_social_max, user_uuid,team_uuid)
    AS (
	SELECT 
    date(`agent-task-records`.started_at) as daily,
    ...
    allanswer (daily,answer, uninterest, inbound_incoming, user_uuid,team_uuid)
    AS (
		SELECT
        date(`task-records`.started_at) as daily,
    ...
    alloutbound (daily,incoming,answer_out,no_answer,user_uuid,team_uuid)
    AS (
		SELECT
        date(`task-records`.started_at) as daily,
    ...
    allmiss (daily,miss, user_uuid,team_uuid)
    AS (
		SELECT
        date(`task-records`.started_at) as daily,
		count(uuid) as miss,
...
FROM (
   ...
   ifnull((ifnull(allsocial.handled_social,0)*100)/ifnull(answer,0),0) as handled_social_per
    ...
    left join allanswer on allagent.user_uuid = allanswer.user_uuid and allagent.daily = allanswer.daily and allagent.team_uuid = allanswer.team_uuid
    ...
    left join allmiss on allagent.user_uuid = allmiss.user_uuid and allagent.daily = allmiss.daily and allagent.team_uuid = allmiss.team_uuid
