CREATE DEFINER=`dev`@`%` PROCEDURE `30agentbusytime_noteam`(IN `start_date_param` DATE, IN `end_date_param` DATE, IN `start_time_param` TIME, IN `end_time_param` TIME, IN `agent_param` TEXT)
BEGIN
WITH
    allbreak(num,user_uuid,daily,sum_break,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
	...
	group by `agent-statuses`.user_uuid,date(`agent-statuses`.started_at),`Slot`
    ),
    
	allwork(num,sum_working, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
	...
	alljobassign(num,sum_jobassign, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	allcoaching(num,sum_coaching, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alltyping(num,sum_typing, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	allprivate(num,sum_private, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alltraning(num,sum_traning, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alllunch(num,sum_lunch, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	allsick(num,sum_sick, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alloutbound(num,sum_outbound, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
    ...
    allcomplain(num,sum_complain, user_uuid,daily,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alltotal(user_uuid,agent,daily,sum_total,num,Slot,New_Slot,Slot_count)
    AS (
    SELECT 
	`agent-statuses`.user_uuid,
    ...
    from alltotal
	left join allwork on alltotal.user_uuid = allwork.user_uuid and alltotal.daily = allwork.daily and alltotal.Slot=allwork.Slot  
    left join alljobassign on alltotal.user_uuid = alljobassign.user_uuid and alltotal.daily = alljobassign.daily and alltotal.Slot=alljobassign.Slot
    left join allcoaching on alltotal.user_uuid = allcoaching.user_uuid and alltotal.daily = allcoaching.daily and alltotal.Slot=allcoaching.Slot 
    left join alltyping on alltotal.user_uuid = alltyping.user_uuid and alltotal.daily = alltyping.daily and alltotal.Slot=alltyping.Slot 
    left join allprivate on alltotal.user_uuid = allprivate.user_uuid and alltotal.daily = allprivate.daily and alltotal.Slot=allprivate.Slot 
    left join alltraning on alltotal.user_uuid = alltraning.user_uuid and alltotal.daily = alltraning.daily and alltotal.Slot=alltraning.Slot 
    left join alllunch on alltotal.user_uuid = alllunch.user_uuid and alltotal.daily = alllunch.daily and alltotal.Slot=alllunch.Slot
    left join allsick on alltotal.user_uuid = allsick.user_uuid and alltotal.daily = allsick.daily and alltotal.Slot=allsick.Slot
    left join alloutbound on alltotal.user_uuid = alloutbound.user_uuid and alltotal.daily = alloutbound.daily and alltotal.Slot=alloutbound.Slot 
    left join allbreak on alltotal.user_uuid = allbreak.user_uuid and alltotal.daily = allbreak.daily and alltotal.Slot=allbreak.Slot
    left join allcomplain on alltotal.user_uuid = allcomplain.user_uuid and alltotal.daily = allcomplain.daily and alltotal.Slot=allcomplain.Slot
    ORDER BY alltotal.agent, alltotal.daily, alltotal.Slot
    ;
