...
cross join (select count(uuid) AS alltotal from `task-records` where  date(`task-records`.started_at) between $P{start_date} and $P{end_date}) tr
...
group by `categories`.name, date(`task-records`.started_at),Slot
order by  date(`task-records`.started_at), Slot,`categories`.name
;