CREATE DEFINER=`dev`@`%` PROCEDURE `dailyagentbusytime`(IN `start_date_param` DATE, IN `end_date_param` DATE, IN `business_units_param` TEXT, IN `team_param` TEXT, IN `agent_param` TEXT)
BEGIN
WITH

    allbreak(num,user_uuid,team_uuid,daily,sum_break)
    AS (
    SELECT 
	...
	allwork(num,sum_working, user_uuid,team_uuid,daily)
    AS (
    SELECT 
	...
	alljobassign(num,sum_jobassign, user_uuid,team_uuid,daily)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	allcoaching(num,sum_coaching, user_uuid,team_uuid,daily)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alltyping(num,sum_typing, user_uuid,team_uuid,daily)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	allprivate(num,sum_private, user_uuid,team_uuid,daily)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alltraning(num,sum_traning, user_uuid,team_uuid,daily)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alllunch(num,sum_lunch, user_uuid,team_uuid,daily)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	allsick(num,sum_sick, user_uuid,team_uuid,daily)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alloutbound(num,sum_outbound, user_uuid,team_uuid,daily)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	allcomplain(num,sum_complain, user_uuid,team_uuid,daily)
    AS (
    SELECT 
    count(`agent-statuses`.uuid),
	...
	alltotal(business_unit_uuid,business_units,team_uuid,teams,user_uuid,agent,daily,sum_total,num)
    AS (
    SELECT 
    ...
    from alltotal
	left join allwork on alltotal.user_uuid = allwork.user_uuid and alltotal.daily = allwork.daily and alltotal.team_uuid = allwork.team_uuid
    left join alljobassign on alltotal.user_uuid = alljobassign.user_uuid and alltotal.daily = alljobassign.daily and alltotal.team_uuid = alljobassign.team_uuid
    left join allcoaching on alltotal.user_uuid = allcoaching.user_uuid and alltotal.daily = allcoaching.daily and alltotal.team_uuid = allcoaching.team_uuid
    left join alltyping on alltotal.user_uuid = alltyping.user_uuid and alltotal.daily = alltyping.daily and alltotal.team_uuid = alltyping.team_uuid
    left join allprivate on alltotal.user_uuid = allprivate.user_uuid and alltotal.daily = allprivate.daily and alltotal.team_uuid = allprivate.team_uuid
    left join alltraning on alltotal.user_uuid = alltraning.user_uuid and alltotal.daily = alltraning.daily and alltotal.team_uuid = alltraning.team_uuid
    left join alllunch on alltotal.user_uuid = alllunch.user_uuid and alltotal.daily = alllunch.daily and alltotal.team_uuid = alllunch.team_uuid
    left join allsick on alltotal.user_uuid = allsick.user_uuid and alltotal.daily = allsick.daily and alltotal.team_uuid = allsick.team_uuid
    left join alloutbound on alltotal.user_uuid = alloutbound.user_uuid and alltotal.daily = alloutbound.daily and alltotal.team_uuid = alloutbound.team_uuid
    left join allbreak on alltotal.user_uuid = allbreak.user_uuid and alltotal.daily = allbreak.daily and alltotal.team_uuid = allbreak.team_uuid
    left join allcomplain on alltotal.user_uuid = allcomplain.user_uuid and alltotal.daily = allcomplain.daily and alltotal.team_uuid = allcomplain.team_uuid
    ORDER BY alltotal.business_units, alltotal.teams, alltotal.agent, alltotal.daily
    ;
