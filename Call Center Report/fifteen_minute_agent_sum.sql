CREATE DEFINER=`dev`@`%` PROCEDURE `fifteen_minute_agent_sum`(IN `startDate` DATE, IN `endDate` DATE, IN `startTime` TIME, IN `endTime` TIME, IN `setInterval` INT, IN `bu_param` TEXT, IN `team_param` TEXT, IN `agent_param` TEXT)
BEGIN
-- 	SET GLOBAL time_zone = 'Asia/Bangkok';
	...
	tasks (Date,Business_units,team,agent,Slot,Count,inbound_incoming,answer,abandon,uninterest,outbound_incoming,answer_out,no_answer,inservice,hold,waiting,no_of_hold,avg_inservice,avg_hold,avg_queue) AS
		(
			SELECT 
			...
			GROUP BY `task-records`.`business_unit_uuid`, `teams`.`uuid` , `task-records`.user_uuid, `Date` , `Slot`
			ORDER BY `business-units`.`name`, `teams`.`name` , agent, `Date`
		)
        
		SELECT all_list.b,all_list.c,all_list.d,all_list.slot,all_list.label,tasks.agent,
        ...
        group by all_list.b,all_list.c,tasks.agent, all_list.d, all_list.slot
		ORDER BY all_list.b,all_list.c,tasks.agent, all_list.d, all_list.slot ;
END